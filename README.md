...
- [x] 편의점 안내 메시지를 출력한다.
- [] 보유하고 있는 재고 목록과 행사 목록을 출력한다.
  - [] public/products.md과 public/promotions.md 파일를 이용하여 불러온다.
  - [] 두 파일 모두 내용의 형식을 유지한다면 값은 수정할 수 있다.
- [] 사용자에게 구매할 상품과 수량을 입력받는다.
  - [] 이때 상품명, 수량은 하이픈(-)으로, 개별 상품은 대괄호([])로 묶어 쉼표(,)로 구분한다. `[콜라-10],[사이다-3]`
  - [] 각 상품의 재고 수량을 고려하여 결제 가능 여부를 확인하고, 구매한 상품은 해당 상품의 재고에서 차감한다.
  - [] 재고를 차감함으로써 시스템은 최신 재고 목록을 유지한다.
- [] 프로모션 확인
  - [] 현재 날짜가 프로모션 기간 내에 포함된 경우에만 할인을 적용한다.
  - [] 프로모션은 N개 구매 시 1개를 무료로 제공한다.
...
- [] 구매할 상품과 수량 입력
  ...
  - [] 재고에 없는 상품을 입력한 경우
  - [] 구매 수량이 재고 수량보다 많은 경우
  - [] 구매 수량을 0 이하로 입력한 경우
  - [] 구매할 상품과 수량에 누락된 값이 있는 경우
- [] Y/N 입력(소문자까지 허용)
  - [] Y나 N 이외의 값을 입력한 경우
